...
export function buildPhase10PayoutConfig(input: Phase10ConfigInput = {}): Phase10PayoutConfig {
  ...
  return {
    ...
    primaryProviderUrl: emptyToNull(input.primaryProviderUrl),
    secondaryProviderUrl: emptyToNull(input.secondaryProviderUrl),
    providerApiKey: emptyToNull(input.providerApiKey),
...
export function listPhase10MissingResources(config: Phase10PayoutConfig): string[] {
  ...
  if (config.primaryProviderUrl === null) {
