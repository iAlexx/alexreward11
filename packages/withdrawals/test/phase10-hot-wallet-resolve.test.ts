...
describe.skipIf(phase7DatabaseUrl === '')(
  'phase10 payout Hot Wallet resolve (mode-specific)',
  () => {
    let pool: Pool;
    ...
    it('fake=true + only FALLBACK_ENCRYPTED → rejected', async () => {
      ...
      await expect(
        ...
        code: 'CONFIG',
        message: expect.stringMatching(/No eligible test Hot Wallet/) as unknown as string,
      });
    });
    ...
    it('fake=false + only TEST_ONLY_FAKE% → rejected', async () => {
      ...
      await expect(
        ...
        message: expect.stringMatching(
          /No eligible encrypted payout Hot Wallet/,
    ...
    it('multiple mode-eligible ACTIVE wallets → rejected', async () => {
      ...
      await expect(
        ...
        code: 'CONFIG',
        message: expect.stringMatching(/Ambiguous eligible test Hot Wallets/) as unknown as string,
      });
      ...
      await expect(
        ...
        code: 'CONFIG',
        message: expect.stringMatching(
          /Ambiguous eligible encrypted payout Hot Wallets/,
        ) as unknown as string,
      });
    });
