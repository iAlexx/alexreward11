...
10. Attempts + dispatch fencing; possible-broadcast → `RECONCILE_REQUIRED` (Reserved untouched);
    append-only `withdrawal_payout_reconciliations` derived from authoritative observation only.
...
| authority corrections (config/network/entitlement/reconcile) | 16    |

Prior Phase 7 baseline of **70** remains green; **+8** Temporal; **+16** authority.
