...
### Consolidated Master Specification — Version 1.3

**Status:** Consolidated implementation baseline with V1.3 Phase 9 signer-custody amendment  
**Date:** 2026-09-10  
...
# CHANGELOG — V1.2 TO V1.3 (PHASE 9 SIGNER CUSTODY AMENDMENT)

Version 1.3 is a **surgical Phase 9 signer-custody amendment**. It preserves all non-custody product, financial, and security requirements from Version 1.2.

**Owner Hot Wallet signing-custody decision:**

| Item                                                 | Status                                                                   |
| ---------------------------------------------------- | ------------------------------------------------------------------------ |
| AWS KMS Ed25519 compatibility                        | **TECHNICALLY PROVEN** (historical Phase 9 evidence)                     |
| AWS as production Hot Wallet signing custody backend | **OWNER REJECTED** (operational/vendor-risk after account suspension)    |
| Production Hot Wallet signer target                  | **`apps/signer` + SELF-HOSTED ENCRYPTED Ed25519 (`FALLBACK_ENCRYPTED`)** |

Normative effects of this amendment:

- Preferred / production Hot Wallet signer custody is self-hosted encrypted Ed25519 under the separate `apps/signer` trust boundary (`FALLBACK_ENCRYPTED`).
- AWS KMS spike checklists and compatibility notes are retained as **HISTORICAL / non-authoritative engineering evidence only**.
- The **mandatory production gate** is a formally reviewed, Testnet-validated self-hosted encrypted signer — not AWS KMS approval.
- Invariant language is updated so only `apps/signer` may access usable Hot Wallet private signing capability.
- Disaster-recovery Hot Wallet compromise steps disable signer unlock / revoke encrypted-bundle access (not “Disable KMS” as the production control).

All Version 1.3 custody language supersedes conflicting Version 1.2 Hot Wallet signer-custody / AWS-KMS production-target language. All Version 1.2 requirements not expressly changed by this amendment remain fully in force.
...
- Signer -> isolated `apps/signer` security boundary with the only application path to usable Hot Wallet private signing capability (production custody is not AWS KMS)
- PostgreSQL -> AWS RDS Multi-AZ
- Redis -> AWS ElastiCache
- Temporal -> Temporal Cloud
- Secrets -> AWS Secrets Manager
- Signer key -> SELF-HOSTED ENCRYPTED Ed25519 (`FALLBACK_ENCRYPTED`) target; AWS KMS is OWNER REJECTED as production Hot Wallet signing custody
...
- Production Hot Wallet signing material and encrypted-signer unlock/bundle access are independent of non-production environments (AWS KMS is not the production custody backend).
...
## 34.1 Preferred / production target

Separate `apps/signer` security boundary using **SELF-HOSTED ENCRYPTED Ed25519 (`FALLBACK_ENCRYPTED`)**.

AWS KMS Ed25519 compatibility is **TECHNICALLY PROVEN** as historical Phase 9 evidence, but AWS as production Hot Wallet signing custody is **OWNER REJECTED** (operational/vendor-risk after account suspension).

Desired:

- private key material is envelope-encrypted at rest and never stored in plaintext in repo/frontend/database;
- decrypt/unlock occurs only in signer memory for the duration of an authorized sign operation;
- only the signer service may access usable Hot Wallet private signing capability;
- API, bot, Admin, general Temporal workers and payout dispatcher have no access to decrypted key material, unlock capability, or signing;
...
3. active Hot Wallet and exact key/version identity for the self-hosted encrypted signer;
...
## 34.2 AWS KMS compatibility spike — HISTORICAL / non-authoritative

**Status:** HISTORICAL engineering evidence only. **Not** an authoritative production custody requirement and **not** a mandatory production gate.

Do not treat AWS KMS as the production Hot Wallet signing backend solely because TON and KMS support Ed25519, or because the spike succeeded.

Historical spike checklist (retained for evidence; completed as engineering investigation):
...
**Mandatory production gate:** formally reviewed, Testnet-validated self-hosted encrypted signer (`FALLBACK_ENCRYPTED`) under `apps/signer` — see §34.3.

## 34.3 Production signer — SELF-HOSTED ENCRYPTED (`FALLBACK_ENCRYPTED`)

Production Hot Wallet signing MUST use the self-hosted encrypted path (formerly described as the fallback when KMS was the preferred target):

- dedicated `apps/signer` signing service;
- dedicated mnemonic / Ed25519 key material;
- envelope encrypted at rest;
- decrypt only in signer memory;
- no logs of key material;
- restricted network and host/access controls;
- no DB mutation capability.

This mode preserves the same independent policy-validation boundary. It must not return signing permission to the general payout worker.

Before mainnet/production enablement, the self-hosted encrypted signer must be formally reviewed and Testnet validated.
...
1. Pause payouts.
2. Disable signer unlock / revoke encrypted-bundle access (and revoke any residual historical KMS signing permission if present).
3. Alert Owner.
...
```text
Only apps/signer may access usable Hot Wallet private signing capability
```
...
| Signer boundary            | Wrong role, state, wallet, recipient, amount, asset, master, attempt, query ID, seqno, limits or hash                      | No signature; only `apps/signer` reaches usable Hot Wallet private signing capability |
| TON Testnet                | Self-hosted encrypted signer (`FALLBACK_ENCRYPTED`) signing, Wallet V5, TEP-74 success, seqno, bounce/failure and query ID | Intended successful Jetton transfer is proven                                         |
...
wallet seed phrases
private keys
Hot Wallet private/signing material (including any historical KMS signing material)
...
6. formally reviewed and Testnet-validated self-hosted encrypted Ed25519 signer (`FALLBACK_ENCRYPTED`) under `apps/signer` (AWS as production Hot Wallet signing custody is OWNER REJECTED; the historical AWS KMS Ed25519 spike remains non-authoritative evidence only);
...
Build/validate the separate `apps/signer` boundary for Hot Wallet signing.

**Production custody target (normative):** `apps/signer` + SELF-HOSTED ENCRYPTED Ed25519 (`FALLBACK_ENCRYPTED`). AWS as production signer custody backend is **OWNER REJECTED** (operational/vendor-risk after account suspension).

**Historical evidence (non-authoritative):** AWS KMS Ed25519 compatibility is **TECHNICALLY PROVEN**. The AWS KMS spike checklist in §34.2 is retained as historical engineering evidence only and is **not** a mandatory production gate.

General API/Bot/Admin/Worker processes must not have access to usable Hot Wallet private signing capability.

Test (production-path — self-hosted encrypted signer):
...
- key material envelope-encrypted at rest; decrypt only in signer memory; unlock/bundle access restricted to `apps/signer`.

Historical AWS KMS spike evidence may be referenced for engineering history only; it does not authorize AWS KMS as production custody.

**Mandatory production gate:** formally reviewed, Testnet-validated self-hosted encrypted signer (`FALLBACK_ENCRYPTED`).
...
| General worker attempts Hot Wallet signing                         | Access denial; Critical security alert                                               |
...
- [ ] self-hosted encrypted signer (`FALLBACK_ENCRYPTED`) formally reviewed and Testnet validated
- [ ] only `apps/signer` may access usable Hot Wallet private signing capability
...
33. Separate signer boundary independently validates the canonical attempt and signs via self-hosted encrypted Ed25519 (`FALLBACK_ENCRYPTED`).
...
## AWS KMS — HISTORICAL / non-authoritative engineering evidence
...
The historical AWS KMS spike checklist (§34.2) is non-authoritative evidence only. The mandatory production gate is a formally reviewed, Testnet-validated self-hosted encrypted signer.
...
8. Produce TON self-hosted encrypted signer (`FALLBACK_ENCRYPTED`) Testnet validation plan (AWS KMS spike notes are historical evidence only).
...
- expose Hot Wallet private signing capability to normal API;
